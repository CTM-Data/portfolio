...
def main():
    ...
    while running: 
        ...
        position_input = str(input('Please enter your players position (RB, WR, TE): ').upper())
        touch_g_input = float(input('What is your player"s touches/g metric? '))
        prediction = model_dict[position_input].predict([[touch_g_input]])
        print(f'Fantasy PPG rediction for {player_name_input} ({position_input}): {prediction}')
